...
interface NotebookPreviewSimpleProps {
  cells: CellState[];
}
 ...
 */
export function NotebookPreviewSimple({ cells }: NotebookPreviewSimpleProps) {
  return (
          ...
          {cells.map((cell) => {
            if (cell.type === 'markdown') {
              return (
                <div key={cell.id} className="markdown-preview">
                  {cell.title?.trim() && <h3 className="mb-2 text-base font-semibold">{cell.title}</h3>}
            ...
            return (
              <div key={cell.id} className="space-y-2">
                {cell.title?.trim() && <h3 className="text-base font-semibold text-quack-dark">{cell.title}</h3>}
